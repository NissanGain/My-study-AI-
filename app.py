...
with tab2:
    ...
    st.write("Analyzes syllabus and board trends.")
    bp_search = st.toggle("Search latest 2026 syllabus & sample papers?, most probable question", value=True, key="bp_search")
    ...
    if st.button("Predict High-Weightage Topics"):
        with st.spinner("Scanning board updates..."):
            context = ""
            if bp_search:
                search_query = f"Class 10 {subject} 2026 board exam pattern and syllabus updates"
                context = f"Latest Board News: {get_web_context(search_query, max_results=10)}\n\n"
            
            # Using the smarter model for predictions
...
with tab3:
    ...
    if st.button("Generate Questions"):
        with st.spinner("Searching recent papers..."):
            context = ""
            if pyq_search:
